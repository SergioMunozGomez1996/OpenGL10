...
public class OpenGLRenderer implements Renderer {
	...
	// Multiplicador del factor de escalado / zoom
	private float tZ = DEFAULT_ZOOM;
	...
	private int	texture;
	private int	textureCrown;

	// Rotación alrededor de los ejes
	private float rX = 0f;
	private float rY = 0f;

	// Rotación eje y para la corona
	private float rYCrown = 0f;

	//Ultima posicion seekbar
	float lastSeekBarPosition  = 0f;
	...
	public OpenGLRenderer(Context context) {
		...
		obj3DS.read3DSFromResource(context, R.raw.king_boo);

		obj3DS2 = new Resource3DSReader();
		obj3DS2.read3DSFromResource(context, R.raw.king_boo_crown);
	}
	...
	public void onSurfaceCreated(GL10 glUnused, EGLConfig config) {
		...
		texture = TextureHelper.loadTexture(context, R.drawable.boo_color);
		textureCrown = TextureHelper.loadTexture(context, R.drawable.crown_color);
	...
	public void onDrawFrame(GL10 glUnused) {
		...
		// Ponemos el multiplicador de escalado
		translateM(modelMatrix, 0, 0f, 0.0f, -tZ);
		...
		rotateM(modelMatrix, 0, rY , 0f, 1f, 0f);
		rotateM(modelMatrix, 0, rX , 1f, 0f, 0f);
		rotateM(modelMatrix, 0, rYCrown , 0f, 1f, 0f);
		...
		// Pasamos la textura
		glActiveTexture(GL_TEXTURE0);
		glBindTexture(GL_TEXTURE_2D, textureCrown);
		glUniform1f(uTextureUnitLocation, 0);

		for (int i=0; i<obj3DS2.numMeshes; i++) {
			// Asociando v?rtices con su attribute
			obj3DS2.dataBuffer[i].position(0);
			glVertexAttribPointer(aPositionLocation, POSITION_COMPONENT_COUNT, GL_FLOAT,
					false, STRIDE, obj3DS2.dataBuffer[i]);

			// Asociamos el vector de normales
			obj3DS2.dataBuffer[i].position(POSITION_COMPONENT_COUNT);
			glVertexAttribPointer(aNormalLocation, NORMAL_COMPONENT_COUNT, GL_FLOAT,
					false, STRIDE, obj3DS2.dataBuffer[i]);

			// Asociamos el vector de UVs
			obj3DS2.dataBuffer[i].position(POSITION_COMPONENT_COUNT+NORMAL_COMPONENT_COUNT);
			glVertexAttribPointer(aUVLocation, NORMAL_COMPONENT_COUNT, GL_FLOAT,
					false, STRIDE, obj3DS2.dataBuffer[i]);
			glDrawArrays(GL_TRIANGLES, 0, obj3DS2.numVertices[i]);
		}
	}
	...
	public void handleTouchDrag(float normalizedX, float normalizedY) {
		...
		float deltaX = normalizedX - lastX;
		float deltaY = normalizedY - lastY;
		rX = rX+deltaY*75;
		rY = rY+deltaX*75;
		lastX = normalizedX;
		lastY = normalizedY;
	...
	public void handleSeekBar(float normalizedY){
		if(lastSeekBarPosition < normalizedY) {
			rYCrown = rYCrown + normalizedY * 0.1f;
		}else {
			rYCrown = rYCrown - normalizedY * 0.1f;
		}
		lastSeekBarPosition = normalizedY;
	}

	// Asignación y retorno del factor de escalado
